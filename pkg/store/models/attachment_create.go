...
func (ac *AttachmentCreate) SetNillableUUID(s *string) *AttachmentCreate {
	if s != nil {
		ac.SetUUID(*s)
	}
	return ac
...
func (ac *AttachmentCreate) check() error {
	if v, ok := ac.mutation.UUID(); ok {
		if err := attachment.UUIDValidator(v); err != nil {
			return &ValidationError{Name: "uuid", err: fmt.Errorf(`models: validator failed for field "Attachment.uuid": %w`, err)}
		}
	}
	if _, ok := ac.mutation.DownloadCount(); !ok {
...
func (ac *AttachmentCreate) createSpec() (*Attachment, *sqlgraph.CreateSpec) {
	...
	if value, ok := ac.mutation.UUID(); ok {
		_spec.SetField(attachment.FieldUUID, field.TypeString, value)
		_node.UUID = value
	}
