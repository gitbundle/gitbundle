...
type Topic struct {
	...
	// RepoCount holds the value of the "repo_count" field.
...
func (*Topic) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case topic.FieldID, topic.FieldRepoCount:
...
func (t *Topic) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case topic.FieldRepoCount:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field repo_count", values[i])
			} else if value.Valid {
				t.RepoCount = int(value.Int64)
			}
...
func (t *Topic) Unwrap() *Topic {
	...
	if !ok {
		panic("models: Topic is not a transactional entity")
	}
...
func (t *Topic) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("repo_count=")
	builder.WriteString(fmt.Sprintf("%v", t.RepoCount))
	builder.WriteString(", ")
