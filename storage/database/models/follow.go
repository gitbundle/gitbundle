...
func (*Follow) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case follow.FieldID, follow.FieldUserID, follow.FieldFollowID:
...
func (f *Follow) Unwrap() *Follow {
	...
	if !ok {
		panic("models: Follow is not a transactional entity")
	}
...
func (f *Follow) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", f.FollowID))
	builder.WriteString(", ")
