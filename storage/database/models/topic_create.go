...
func (tc *TopicCreate) SetRepoCount(i int) *TopicCreate {
	tc.mutation.SetRepoCount(i)
	return tc
}

// SetNillableRepoCount sets the "repo_count" field if the given value is not nil.
func (tc *TopicCreate) SetNillableRepoCount(i *int) *TopicCreate {
	if i != nil {
		tc.SetRepoCount(*i)
	}
	return tc
...
func (tc *TopicCreate) check() error {
	...
	if _, ok := tc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`models: missing required field "Topic.created_at"`)}
	}
	return nil
}
...
func (tc *TopicCreate) createSpec() (*Topic, *sqlgraph.CreateSpec) {
	...
	if value, ok := tc.mutation.RepoCount(); ok {
		_spec.SetField(topic.FieldRepoCount, field.TypeInt, value)
		_node.RepoCount = value
	}
