...
func (tx *Tx) init() {
	...
	tx.HookTask = NewHookTaskClient(tx.config)
	...
	tx.Oauth2Application = NewOauth2ApplicationClient(tx.config)
	tx.Oauth2ApplicationCode = NewOauth2ApplicationCodeClient(tx.config)
	tx.Oauth2Grant = NewOauth2GrantClient(tx.config)
	tx.OrgUser = NewOrgUserClient(tx.config)
	tx.Pkg = NewPkgClient(tx.config)
	tx.PkgBlob = NewPkgBlobClient(tx.config)
	tx.PkgBlobUpload = NewPkgBlobUploadClient(tx.config)
	tx.PkgFile = NewPkgFileClient(tx.config)
	tx.PkgProperty = NewPkgPropertyClient(tx.config)
	tx.PkgVersion = NewPkgVersionClient(tx.config)
	tx.ProtectedBranch = NewProtectedBranchClient(tx.config)
	tx.ProtectedTag = NewProtectedTagClient(tx.config)
	tx.PublicKey = NewPublicKeyClient(tx.config)
	tx.RenamedBranch = NewRenamedBranchClient(tx.config)
	...
	tx.Star = NewStarClient(tx.config)
	...
	tx.Watch = NewWatchClient(tx.config)
